...
class FeeRuleControllerTest {
  ...
  @MockitoBean private BaseFeeRepository baseFeeRepository;
  ...
  void getAllBaseFees_returnsListOfFees() throws Exception {
    ...
    mockMvc
        .perform(get("/api/rules/base-fees"))
  ...
  void createBaseFee_validRequest_returnsSavedFee() throws Exception {
    ...
    BaseFee savedFee =
        BaseFee.builder().id(3L).city("NARVA").vehicleType("SCOOTER").fee(2.0).build();
  ...
  void updateBaseFee_existingId_returnsUpdatedFee() throws Exception {
    Long id = 1L;
    BaseFee existingFee =
        BaseFee.builder().id(id).city("TALLINN").vehicleType("BIKE").fee(3.0).build();
  ...
  void deleteBaseFee_existingId_returnsOk() throws Exception {
    ...
    mockMvc.perform(delete("/api/rules/base-fees/{id}", id)).andExpect(status().isOk());
  ...
  void deleteBaseFee_nonExistingId_returnsNotFound() throws Exception {
    ...
    mockMvc.perform(delete("/api/rules/base-fees/{id}", id)).andExpect(status().isNotFound());
