...
/**
 * Web MVC tests for the CityStationController.
 * Verifies the REST API endpoints for viewing and adding dynamic city-station mappings
...
class CityStationControllerTest {
    ...
    /**
     * Tests the GET /api/cities endpoint.
     * Expects a JSON array containing all configured city-to-station mappings.
     */
    @Test
    void getAllCities_returnsListOfMappings() throws Exception {
        CityStationMapping mapping = CityStationMapping.builder()
                .id(1L).city("TALLINN").stationName("Tallinn-Harku").build();
    ...
    /**
     * Tests the POST /api/cities endpoint.
     * Expects a successful 200 OK response with the saved mapping object in JSON format.
     */
    @Test
    void addCity_validRequest_returnsSavedMapping() throws Exception {
        CityStationMapping newMapping = CityStationMapping.builder()
                .city("KURESSAARE").stationName("Roomassaare").build();

        CityStationMapping savedMapping = CityStationMapping.builder()
                .id(2L).city("KURESSAARE").stationName("Roomassaare").build();
