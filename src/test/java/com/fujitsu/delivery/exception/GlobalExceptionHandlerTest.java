...
/**
 * Unit tests for GlobalExceptionHandler. Uses a standalone MockMvc setup with a dummy controller to
 * verify that exceptions are intercepted and correctly formatted as JSON error responses.
 */
class GlobalExceptionHandlerTest {
  ...
  void handleGeneralException_returns500AndJson() throws Exception {
    mockMvc
        .perform(get("/dummy/general-error"))
  ...
  /**
   * Verifies that malformed JSON requests result in a 400 Bad Request JSON response containing the
   * expected format guide.
   */
  @Test
  void handleMalformedJsonException_returns400AndCustomFormat() throws Exception {
    mockMvc
        .perform(post("/dummy/malformed-json"))
        .andExpect(status().isBadRequest())
        .andExpect(header().string("Content-Type", "application/json"))
  ...
  private static class DummyController {
    ...
    public void throwForbidden() {
      throw new VehicleForbiddenException("Vehicle is forbidden");
    }
