...
/**
 * REST client for interacting with the Estonian National Weather Service API. Handles the low-level
 * HTTP communication to fetch raw weather data.
 */
@Component
...
public class NationalWeatherServiceRestClient {
  ...
  /**
   * Fetches the latest weather observations from the external XML source. * @return
   * WeatherDataXmlDTO.Observations containing the timestamp and list of stations
   *
  ...
  public NationalWeatherServiceWeatherData.Observations fetchObservations() {
    ...
    return restTemplate.getForObject(
        weatherUrl, NationalWeatherServiceWeatherData.Observations.class);
  }
}
