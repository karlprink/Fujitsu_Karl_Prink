...
/**
 * Service responsible for importing weather data from external sources, filtering relevant
 * stations, and managing data persistence.
 */
@Service
...
public class WeatherImportService {
   ...
   * Uses @Transactional to ensure atomicity; if an error occurs during saving, the transaction is
   * rolled back.
   */
  public void importWeatherData() {
    try {
      NationalWeatherServiceWeatherData.Observations observations =
          weatherClient.fetchObservations();
      ...
      Set<String> activeStations =
          cityStationMappingRepository.findAll().stream()
      ...
      List<WeatherData> weatherDataList =
          observations.getStations().stream()
              ...
              .filter(
                  station ->
                      !weatherDataRepository.existsByStationNameAndObservationTimestamp(
  ...
  /**
   * Persists the filtered list of weather data to the database. * @param weatherDataList List of
  ...
  private void saveImportedData(List<WeatherData> weatherDataList) {
    if (!weatherDataList.isEmpty()) {
      ...
      log.info(
          "Successfully imported and saved {} NEW weather data records.", weatherDataList.size());
    } else {
  ...
  /**
   * Logic executed on startup to determine if an immediate import is required. Uses flattened logic
   * instead of deeply nested blocks.
   */
  ...
  /**
   * Checks if the last saved data is older than the configured cron interval. * @param
   * latestTimestamp Timestamp of the most recent database record
   *
  ...
  /**
   * Calculates the duration between scheduled executions based on the cron expression. * @param
   * cronStr The cron expression string
   *
  ...
  /**
   * Maps a station DTO from the XML source to a WeatherData entity. * @param station The DTO from
   * the XML
   *
  ...
  private WeatherData buildWeatherData(
      NationalWeatherServiceWeatherData.Station station, LocalDateTime timestamp) {
