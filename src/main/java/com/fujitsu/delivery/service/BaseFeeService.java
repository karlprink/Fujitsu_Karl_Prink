...
/**
 * Service responsible for managing the delivery base fee rules.
 * Handles the creation, retrieval, updating, and deletion of base fees,
 * ensuring that duplicate rules for the same city and vehicle type are not created.
 */
@Service
...
public class BaseFeeService {
     ...
     * Validates that a rule for the specified city and vehicle type combination does not already exist.
     *
    ...
    public BaseFee createBaseFee(BaseFee baseFee) {
        Optional<BaseFee> existingFee = baseFeeRepository.findByCityIgnoreCaseAndVehicleTypeIgnoreCase(
     ...
     * @return an Optional containing the updated BaseFee if found and saved, or an empty Optional if not found
     */
